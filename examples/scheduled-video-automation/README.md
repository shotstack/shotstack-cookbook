...
Run it again. The item is skipped because it is in `state.json`. Add a second object to `feed.json` with a new `id` and run again to render only the new item.
...
To receive completion callbacks instead of polling, generate a secret and add it with your public HTTPS URL to `.env`:
...
`videos.mjs` reads `feed.json`, or the JSON array at `FEED_URL` when it is set. Each item needs a unique `id`, a `title` and one to four HTTPS image URLs. For each item not in `state.json`, the worker submits a template render with the title and images as merge fields, and records the render ID with status `pending`. It submits at most three new items per run and keeps at most ten renders pending.

On each run the worker checks pending renders. A render is checked when its callback marker exists in `inbox/`, when it has not been checked for 15 minutes, or on every run with `--now`. When the Edit API reports `done` and the Serve API reports the MP4 as `ready`, the entry becomes `done` with the hosted `url`. A render takes about one minute.

`webhook.mjs` listens on `127.0.0.1:3000`. It accepts a POST to `/webhook` only with the correct `token`, reads the render ID from the event and writes an empty marker file to `inbox/`. `enable-callback.mjs` adds the callback URL, with the token, to your saved template.

If the API key is missing or rejected, each script prints one line that tells you what to fix and stops. A rejected submission is saved as `rejected` in `state.json` with the error, so the next run does not resubmit it. Fix the cause and use `--retry` with the item ID.
